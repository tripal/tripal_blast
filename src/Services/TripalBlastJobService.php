...
class TripalBlastJobService {
  ...
  public function jobsCountRecentJobs() {
    return (isset($_SESSION['blast_jobs'])) ? count($_SESSION['blast_jobs']) : 0;
  }
   ...
   *   An array of blast programs you want jobs to be displayed for
   *   (i.e.: blastn, blastx, tblastn, blastp)
   *
   * @return array
   *   An array of recent jobs.
   */
  public function jobsGetRecentJobs(array $programs = []): array {
   ...
   *   Form API table type.
   */
  public function jobsCreateTable(array $program = []): array {
   ...
   *   Numeric unique id of BLAST job request.
   *   Corresponds to job_id in the public.tripal_jobs table.
   * @param array $options
  ...
  public function jobsGetJobByJobId(string|int $job_id, array $options = []): bool|\stdClass {
    ...
    if ($blastjob->target_blastdb) {
      ...
      if ($config) {
        ...
        $job->blastdb->db_url = $config['db_url'];
    ...
    // file type suffix (i.e.: .xml, .tsv, .html, .gff, .asn). To determine the
   ...
   *   - blast_program: The BLAST program to execute
   *     (i.e.: blastn, tblastn, tblastx, blastp, blastx).
   *   - target_blastdb: The tripal_blast_database id of the target database
   *     to search against.
   ...
   *   - result_filestub: The full path to the file where the results will be
   *     written (without file type suffix).
   *   - options: An array of additional options where the key is the name of
  ...
  public function createBlastJob(array $job_parameters): ?int {
    ...
    elseif (array_key_exists('target_blastdb', $job_parameters)) {
      if (is_numeric($job_parameters['target_blastdb'])) {
        ...
        if (!$database_config) {
          ...
          // If the target_blastdb is set then we need to make sure the
          // target_file is set to the path of the database.
    ...
    $job_id = tripal_add_job(
      'BLAST (' . $job_parameters['blast_program'] . '): ' . $job_parameters['query_file'],
      'blast_job',
   ...
   *   The output file paths where the key is the file type (i.e.: 'xml', 'tsv',
  ...
  public function getBlastCommand(string $program, string $query, string $database, array $output_file, array &$options): array {
    ...
    // We are given them in integer pairs (i.e.: 5_2) but BLAST requires them to
    ...
    if (!(file_exists($database . '.nsq') or file_exists($database . '.psq'))) {
      throw new \Exception("Unable to find the BLAST database (i.e.: $database). Please ensure you have supplied the absolute path not including the file format endings.");
    }

    // The BLAST executeable.
    $program = $blast_path . $program;
    if (!file_exists($program)) {
      throw new \Exception("Unable to find the BLAST executable (i.e.: /usr/bin/blastn). This can be changed in the admin settings; you supplied: $program");
    }
    ...
    if (!file_exists($blast_formatter_command)) {
      throw new \Exception("Unable to find the BLAST Formatter executable (i.e.: /usr/bin/blast_formatter). This can be changed in the admin settings; you supplied: $blast_formatter_command");
    }

    // Note: all variables are escaped (adds single quotes around
    // their values) for security reasons.
    ...
    if (!empty($options)) {
      foreach ($options as $opt => $val) {
        $val = trim($val);
        if (!empty($val)) {
          // We want to escape all the option values since they were supplied
          // via user input. These values should also have been checked in the
   ...
   *
   * @param string $program
   *   Which BLAST program to run (i.e.: 'blastn', 'tblastn',
   ...
   *
   * @return string
   *   A string describing the number of sequences and often including
   *   the first query header.
   */
  public function jobsFormatQueryHeaders(string $file): string {
    $headers = [];

    // Retrieve all FASTA header lines.
    exec('grep ">" ' . escapeshellarg($file), $headers);

    // Easiest case: if there is only one query header then show it.
    if (count($headers) == 1 and isset($headers[0])) {
      return ltrim($headers[0], '>');
    }
    // If we have at least one header, then show that along with the
    // count of queries.
    elseif (isset($headers[0])) {
      return count($headers) . ' queries including "' . ltrim($headers[0], '>') . '"';
    }
   ...
   *
   * @todo find a more efficient way since currently the first loop
   *   stores all the blast results into an array and then the second
   *   loop prints them.
   *
   * @param string $blast_tsv
   *   The name of the blast tsv output file.
   * @param string $blast_gff
   *   The name of the blast gff output file.
   *
   * @return void
   *   No return value, writes to output files.
   */
  public function jobsConvertTsvToGff3(string $blast_tsv, string $blast_gff): void {
    ...
    while (!feof($tsv)) {
      ...
      // Determine query strand to use in match_part line, no need to
      // store, just print.
      $q_strand = '+';
      if ($qs > $qe) {
        [$qs, $qe] = [$qe, $qs];
      ...
      // Determine subject (hit) strand to use in match line, needs to
      // be stored.
      ...
      $hitResult["$s,$q"]['HSPs'][] = implode("\t", [
        ...
        $hitResult["$s,$q"]['strand'],
        '.',
        "ID=$s.$q.$hsp;Parent=$s.$q;Target=$q $qs $qe $q_strand",
      ]);
      $last_s = $s;
    }
   ...
   *   the smallest and largest coordinates for the parent.
   *
   * @return void
  ...
  public function jobsPrintGffParentChildren(mixed $gff, array $blast_feature_array): void {
    foreach ($blast_feature_array as $sq => $value) {
      ...
      $parent = implode("\t", [
        ...
        'BLASTRESULT',
        'match',
        $blast_feature_array["$s,$q"]['SS'],
        $blast_feature_array["$s,$q"]['SE'],
        $blast_feature_array["$s,$q"]['E'],
        $blast_feature_array["$s,$q"]['strand'],
        '.',
        "ID=$s.$q;Name=$q($evalue)",
      ]) . "\n";
